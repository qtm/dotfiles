...
class WorkingTime(object):
    ...
    @property
    def minute(self):
        return self.now + relativedelta(second=0)

    @property
    def today(self):
        return self.now + relativedelta(hour=0, minute=0, second=0)

    @property
    def week_start(self):
        return self.today + relativedelta(weekday=MO(-2))
    ...
    def total_business_days_count(self):
        return rrule(DAILY, dtstart=self.week_start, until=self.week_end, byweekday=self.BUSINESS_DAYS).count()
    ...
    def total_days_count(self):
        return rrule(DAILY, dtstart=self.week_start, until=self.week_end, byweekday=self.WEEK_DAYS).count()
    ...
    def business_days_left_count(self):
        return rrule(DAILY, dtstart=self.now, until=self.week_end, byweekday=self.BUSINESS_DAYS).count()
        #return rrule(DAILY, dtstart=self.now, until=self.month_end, byweekday=self.BUSINESS_DAYS).count()

    @property
    def days_left_count(self):
        return rrule(DAILY, dtstart=self.now, until=self.week_end, byweekday=self.WEEK_DAYS).count()
